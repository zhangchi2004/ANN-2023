...
if __name__ == "__main__":
    ...
    parser.add_argument('--interp_nums', default=10, type=int)
    parser.add_argument('--interp_left',default=0, type=float)
    parser.add_argument('--interp_right',default=1, type=float)
    ...
    if args.interp:
        with torch.no_grad():
            interp_imgs = None
            for i in range(10):
                nums = args.interp_nums
                left = args.interp_left
                right = args.interp_right
                
                vec1 = torch.randn(1, netG.latent_dim, 1, device=device)
                vec2 = torch.randn(1, netG.latent_dim, 1, device=device)
                weights = torch.linspace(left, right, nums, device=device)
                vecs = torch.lerp(vec1, vec2, weights)
                vecs = vecs.transpose(1,2).view(nums, netG.latent_dim, 1, 1)
                
                if interp_imgs is None:
                    interp_imgs = netG.forward(vecs)
                else:
                    interp_imgs = torch.cat((interp_imgs, netG.forward(vecs)), 0)
                print(interp_imgs.shape)
            imgs = make_grid(interp_imgs, nrow=nums, pad_value=0) *0.5 + 0.5
            save_image(imgs,"interpolation/interpolation.png")
            exit(0)
    
    if(args.generate):
        with torch.no_grad():
            samples = None
            while samples is None or samples.size(0) < args.sample_num:
                imgs = netG.forward(torch.randn(1, netG.latent_dim, 1, 1, device=device))
                if samples is None:
                    samples = imgs
                else:
                    samples = torch.cat((samples, imgs), 0)
            imgs = make_grid(samples, nrow=10, pad_value=0) *0.5 + 0.5
            save_image(imgs,"samples/samples.png")
            exit(0)
